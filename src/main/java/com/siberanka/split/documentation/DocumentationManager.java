 ...
 * directory. wiki.yml contains comments only, is never parsed as configuration,
 * and cannot affect runtime
 * behavior.
 */
public final class DocumentationManager {

    public static final String DOCUMENTATION_FILE = "wiki.yml";
    private static final String LEGACY_DOCUMENTATION_FILE = "WIKI.md";
    ...
    public boolean sync() throws IOException {
        ...
        Path legacyDestination = plugin.getDataFolder().toPath().resolve(LEGACY_DOCUMENTATION_FILE);
        return syncWithLegacyMigration(bundledDocumentation, destination, legacyDestination);
    }

    static boolean syncWithLegacyMigration(
            InputStream bundledDocumentation,
            Path destination,
            Path legacyDestination
    ) throws IOException {
        boolean firstYamlSync = !Files.exists(destination, LinkOption.NOFOLLOW_LINKS);
        boolean changed = sync(bundledDocumentation, destination);
        // WIKI.md was owned and regenerated by Split 1.1.1. Remove it only during
        // the first wiki.yml migration so unrelated files created later are safe.
        boolean removedLegacy = firstYamlSync && Files.deleteIfExists(legacyDestination);
